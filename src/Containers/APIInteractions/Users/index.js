...
import { Box, Card, CardBody, CardFooter, CardHeader, Heading, Text } from 'grommet';
...
function Users() {
  const { isLoading, error: isError, data: users } = useQuery('fetchUsers', async () => {
          ...
          {users.map((user) => (
            ...
            >
              <Card height="xsmall" background="light-3">
                ...
                <CardFooter pad={{ horizontal: 'small' }} background="harmonie-6">
                  <Text color="brand">Member since</Text>
                  <Text>{DateTime.fromISO(user.created_at).toLocaleString(DateTime.DATETIME_MED)}</Text>
                </CardFooter>
              </Card>
            </Box>
          ))}
